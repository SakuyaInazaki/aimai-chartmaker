#!/usr/bin/env python3
"""配置识别器（**键位版**）：把官方谱正文切成"配置片段"，并给出逐小节配置硬度分。

⚠️ **本模块已被 `configs_hand.py` 取代，保留作对照基线（2026-09-19）**
--------------------------------------------------------------------
用户 2026-09-19 指出：

    「不可能只有定拍、错位、大宇宙是按手定义的啊，**任何配置都应该按手定义**吧。」

本模块只看**键位序列与时间几何**，"手"仅作为隐含代理出现（硬度分的位移项假设
"同一只手 = 隔一个音"）。按手重写的版本是
:mod:`chart_analysis.configs_hand`（规格 `docs/hand-sequencing.md` §5/§5.1），
它直接消费 `hands.assign()` 的左右手分配结果。

**新的分析一律用 `configs_hand.py`**；本模块**保留不删**，因为
`docs/research/` 下三份既有报告（note 047 / 049 / 052）的全部数字都建在它上面，
删掉就无法对照"哪些结论是检测器带来的假象"。
两版在 388 谱上的逐类差异与机制拆解见 `docs/research/configs-hand-rewrite.md`。
