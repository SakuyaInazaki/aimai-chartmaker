...
# 🧪 **n=160 复验（2026-09-13 第三轮，`audio-chart-calibration-n160.md` §4.1）**：
#    池化最优 小节尺度 **0.340**（0.25 的 SSE 只差 **+0.53%** → 小节地板第三次站住，不动）、
#    段落尺度 **0.330**。⚠️ **段落默认 0.60 的代价从 n=40 的 +3.7% 涨到 +9.53%**
#    （0.25 则是 +1.14%）。**仍然不改默认**：0.60 来自 388 谱的段间落差统计（大样本优先），
#    而且它是"只封不抬"的保守值；但代价已经不小，下一轮若再涨应当重新裁定。
SECTION_DENSITY_FLOOR = 0.60
BAR_DENSITY_FLOOR = 0.25
# 段落尺度的池化最优（**仅记录，不参与计算**）：n=8 曾报 0.125（已推翻）、
# n=40 报 0.365、**n=160 报 0.330**。
CALIBRATED_SECTION_FLOOR = 0.330
...
# 🧪 **n=160 复验（2026-09-13 第三轮，n160 报告 §6.3）：0.75 被确认，不动。**
#    155 个 intro 段的「实际密度 / 全曲均值密度」分布是 p25 0.597 / **中位 0.784** /
#    p75 0.975 —— 0.75 落在中位稍下，是个合理的保守值。
#    ⚠️ **33/155（21.3%）的 intro 比全曲均值还密**（p90 1.073）；
#    MYTHOS 型故障在 187 个 intro/outro 段里只有 **6 段（3.2%）**，比 n=40 看到的还低。
