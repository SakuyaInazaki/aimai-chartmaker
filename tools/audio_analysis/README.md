...
## 0.-1 n=160 复验一览（2026-09-13 第三轮，**只改了两处记录值，行为没变**）

依据：`docs/research/audio-chart-calibration-n160.md`。用户再提供 120 首官方 ST 曲包，
样本扩到 **160 首**（定数 13.0–14.5、BPM **79–266**、官方六曲风各有覆盖）。

| 项 | n=160 的裁定 |
|---|---|
| **强度融合权重 `[0,0.19,0.07,0,0.74]`** | **不再换**。LOSO 160 折对照现役默认：**−0.0015 / 82 折胜（51.3%）/ p = 0.81**，三条判据全不过。对照 v0.3 初值仍然全过（+0.0617 / 77.5% / p≈0）→ **n=40 那次替换被确认正确** |
| **`stemplan.VOCAL_LED_SHARE = 0.14`** | **不动**。1401 段扫出的 Youden 最优是 **0.1424**（n=40 是 0.142），现役值与最优只差 0.007 |
| **`INTRO_OUTRO_CAP_RATIO = 0.75`** | **确认**。155 个 intro 段的实际密度比中位 **0.784**，0.75 落在中位稍下 |
| **`BAR_DENSITY_FLOOR = 0.25`** | **确认**。n=160 池化最优 0.340，默认只差 **0.53%** SSE |
| `CALIBRATED_SECTION_FLOOR`（记录用） | 0.365 → **0.330**。⚠️ 默认 `SECTION_DENSITY_FLOOR = 0.60` 的 SSE 代价从 3.7% 涨到 **9.5%**，仍不改（388 谱大样本优先） |
| `stemplan` 规则 8（`outro` 与 `intro` 同轨） | **从"存疑"升级为"确认的缺陷"**：32 段上仍输常数基线 **9.4 个百分点**。换成什么还没有证据，**行为不动**，注释已写清 |

**n=160 推翻/下修的三条结论**（细节见报告 §7）：

1. **「强度管线对人声曲几乎不管用」说过头了**：器乐/人声的 ρ 倍数从 **2.73× 下修到 1.49×**
   （器乐 0.4196 / 人声 0.2823）。2×2 分解显示 n=40 的 2.73× 被"平滑口径 + v0.3 权重"双重放大。
2. **「锚点应按 BPM 选」（n=40 的 R7）撤回**：NPS 锚在 160 首上于**每一个** BPM 档与定数档
   持平或占优（全样本 MAPE **12.5% vs 15.5%**）。
3. **三条段落类型的"全表最高"措辞被推翻**：`other` 最强的是 `intro`（0.359）不是 `pre_chorus`；
   `interlude` 的 vocals precision（0.561）不再是全表最高；`quiet_chorus` 的 drums recall
   （0.682）略低于 `chorus`（0.692）。

**新增证据**：`piano` 轨在 160 首上确认为**第二有效的踩音轨**
（recall **0.289** > `other` 0.268 > `vocals` 0.197、precision **0.606**、lift **2.44**）
→ 建议把它纳入点缀轨候选（报告 R20，**尚未实现**）；`fx` 仍然很弱（recall 0.057、
lift 1.69，且 24/160 首根本没有 fx 事件）。
...
| **C** | **段落密度地板的 n=8 分歧解除** | `CALIBRATED_SECTION_FLOOR` 0.125 → 0.365（**n=160 再更新为 0.330**；默认 `SECTION_DENSITY_FLOOR = 0.60` **不动**） | 报告 n40 §4.1 / n160 §4.1：池化最优 0.365 → 0.330，默认值的 SSE 代价 3.7% → **9.5%**；**0.125 是 n=8 噪声** |
...
（n=40 实测 16.8%、**n=160 实测 16.0%**，差别无意义）、
`BAR_DENSITY_FLOOR = 0.25`（n=40 最优 0.350、**n=160 最优 0.340**，默认只差 0.5% SSE）。
...
6. **强度融合权重已是 n=40 标定值，但五票权重仍完全未标定**。权重换值的依据见 §0-A。
   ...
   **从未标定**；② 新权重让 `I_bar` 实质变成谱通量曲线（flux 74%），"强度"的语义变窄；
   ③ 换完之后逐小节 Spearman 也只有 **0.37**（n=160 同权重口径 0.3665），**仍然只是中等相关**。
   🧪 **n=160：不再换权重**（对照现役默认三条判据全不过），但 n=40 那次替换被确认正确。
6'. **强度管线对人声曲差三成** ❗（n=40 曾写成"几乎不管用"，**n=160 下修**）：逐小节 ρ
   **器乐曲 0.4196 / 人声曲 0.2823**（51 vs 109 首，倍数 **1.49×**）。
   n=40 报的 0.399 / 0.146（2.73×）被证明是"平滑口径 + v0.3 权重"双重放大的结果。
   人声曲的谱面密度跟着**唱词句读**走，而唱词句读既不在响度里也不在谱通量里 ——
   方向成立，但**没有当初以为的那么致命**。
7. **"建议 note/小节"精度只到档位**：**n=160 逐小节 MAE 均值 2.96** note/小节
   （n=40 是 3.10）→ 它是 **±3 note 的粗档**。
   🧪 **n=160 推翻了 n=40 的"两头不讨好"**：NPS 主锚在全样本上是 **12.5% vs 15.5%**，
   而且在**每一个** BPM 档（<150 打平 / 150–179 10.5 vs 11.2 / 180–199 10.7 vs 10.6 打平 /
   ≥200 **10.5 vs 26.4**）与**每一个**定数档上持平或占优 →
   **主锚维持 NPS，"按 BPM 选锚"的建议撤回**。（n=40 那条是 180–199 档只有 8 首造成的。）
   段落 floor 的 0.60↔0.125 分歧**已解除**（n=40 最优 0.365、n=160 最优 0.330；0.125 是 n=8 噪声）。
8. **intro/outro 的结构封顶比例 0.75（v0.4 由 0.60 上调）**：依据是 39 首的 intro 实际密度/
   ...
   （🧪 **n=160：187 个 intro/outro 段里 6 段（3.2%）**，比例更低；
   intro 实际密度比的中位从 0.726 升到 **0.784**，0.75 仍然落在中位稍下 → **维持**）。
...
9'. **`voiced_ratio` 是个坏掉的探测器** ❗：相对 dB 阈值让 vocals 轨近乎静音的器乐曲拿到
   ...
   （🧪 **n=160：31/51 首，仍然一首都没判反方向**）。
   ⚠️ **n=160 让代理判据的失败模式更刺眼**：器乐曲《疾走あんさんぶる》的 vocals 轨
   拿到 **recall 0.579（160 首最高）** —— 比《超絶！Superlative》的 0.339 还离谱。
