...
- [x] **配置知识重开与采音钻研（2026-09-19，用户指令）**：
  ...
  - **真实出题测试《Dear Player 2》**（note 064–067，2026-09-20 晚）：B 站官方 MV 匿名取音频 → 自动 BPM/offset → 分析 → 写谱 `samples/test-01/` → 桌面包交付；用户在 Visual Maimai 里看到"很多无理 + 全都没对齐"。返工过程：三路 tempo map（滑窗 onset-fit / librosa / beat_this）先判 **205 恒定**（一度提出看 MV 画面定相位被用户否决——**拍相位只能听音乐**）→ **被用户推翻**：用户给的别处抄谱 BPM 表（只读 `(bpm)` 标记，**禁止抄配置**）显示前奏 195→逐小节减速→125→155、尾奏 221→197→190→165→175→135→108，205 恒定只在中段 104 小节成立；错因 = 把无鼓的前奏/尾奏标"不可测"后仍按 205 外推并宣称恒定、把尾奏窗里量到的先升后降当成检测失效 → tempo map 按音频逐段复核重做，前奏/尾奏小节重排（note 067 更正 + note 068）；"歪"的真因 = **VM 吞掉 `||` 注释行之后的行首 `{x}`**（官谱 405/414 行首 `{x}`、0 份含 `||`）→ 交付版改官方格式无注释；复现 MaiMuriDX/MiaCode 静态无理为检查器第七层（160 首官谱逐首与上游 CLI 比对一致），本谱 24 处撞尾（病根：接力头排到第二拍，知识 094）改 17 小节清零、note 数不变；修 `&first≠0` 采音时钟、hands CLI 吃 meta 头、lv 半角 `+` 误报；旧 offset 复核对整格错位与 >0.4% BPM 误差不敏感 → `grid.py` 复核法重做。
