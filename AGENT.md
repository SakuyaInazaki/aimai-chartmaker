...
- [ ] **待用户拍板**：① CC BY 4.0 是否纳入依赖白名单（决定 SongFormer 去留）；② 知识 002「副歌全踩人声」仍存疑需听审；③ 手序对齐余项（note 055 §8）：出张是否分档（现单档代价 0.90）、侧边双押"位移大"的阈值（官谱标不出线，改 3 会标 20 次）、"跨共享键的交叉"算不算反手/出张（ep2 谱例 403/314 在 064 口径下不再命中）、"稍快"上界与 ≤3 tap 是否含 hold/星星头；④ 采音术语阈值（踩掉七成算不算全采）与大宇宙判据口径；⑤ 知识草案入库裁定：note 047/049/050/052/053/055 报告末尾的 NNN 草案（采音模式、密与难两旋钮、推荐矩阵、定数升档规律、手序模型四条、"突然"阈值互换预算等）
