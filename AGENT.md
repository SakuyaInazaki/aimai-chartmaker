...
2. **制谱经验**（`.agent/knowledge/`）：从官方谱与教程沉淀的制谱意识与规范，模型创作时的依据（**已建库 67 条**：001–032 用户逐条讲授 + 官方谱实证与实测规律；033–046 来自 B 站 wasllll《配置术语》第二期、047–063 来自第三期星星篇——**星星配置的讲授来源即该视频**（用户 2026-09-19 指定），置信度按"社区教程"标、用户可覆盖；**064–067 用户手序讲授**（舒适区与出张 / 星星头与条可分属两手 / 手法非写谱导向 / 段首出手与位置连续），分配器 v0.2 以此为准）；
...
- [ ] **待用户拍板**：① CC BY 4.0 是否纳入依赖白名单（决定 SongFormer 去留）；② 知识 002「副歌全踩人声」仍存疑需听审；③ **手序对齐余项：仅 064 口径下两段谱例的归类**（`403-最終鬼畜妹・一部声` MAS m060–063 算不算出张、`314-FLOWER` Re:MAS m071–072 算不算反手——两段跨的全是共享键；**已展示谱例待答**）；④ 知识草案入库裁定：note 047/049/050/052/053/055 报告末尾的 NNN 草案（去量化后的定性版，见 note 056）
