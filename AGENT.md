...
- [x] **配置知识重开与采音钻研（2026-09-19，用户指令）**：
  ...
  - **真实出题测试《Dear Player 2》**（note 064–067，2026-09-20 晚）：B 站官方 MV 匿名取音频 → 自动 BPM/offset → 分析 → 写谱 `samples/test-01/` → 桌面包交付；用户在 Visual Maimai 里看到"很多无理 + 全都没对齐"。返工结论：三路 tempo map（滑窗 onset-fit / librosa / beat_this）定案 **205 恒定、`&first=1.349` 不变、205 = 拍速**（一度提出看 MV 画面定相位被用户否决——**拍相位只能听音乐**）；"歪"的真因 = **VM 吞掉 `||` 注释行之后的行首 `{x}`**（官谱 405/414 行首 `{x}`、0 份含 `||`）→ 交付版改官方格式无注释；复现 MaiMuriDX/MiaCode 静态无理为检查器第七层（160 首官谱逐首与上游 CLI 比对一致），本谱 24 处撞尾（病根：接力头排到第二拍，知识 094）改 17 小节清零、note 数不变；修 `&first≠0` 采音时钟、hands CLI 吃 meta 头、lv 半角 `+` 误报；旧 offset 复核对整格错位与 >0.4% BPM 误差不敏感 → `grid.py` 复核法重做。
...
- [ ] **待用户拍板**：① CC BY 4.0 是否纳入依赖白名单（决定 SongFormer 去留）；② 知识 002「副歌全踩人声」仍存疑需听审；③ 手序对齐余项：064 口径下 `403-最終鬼畜妹・一部声` MAS m060–063 / `314-FLOWER` Re:MAS m071–072 算不算出张/反手（用户 2026-09-20 指示暂不问，agent 按两口径并列记录）；④ 知识 068–094 为 agent 观察条目，用户随时可覆盖或废弃；⑤ 测试谱 `samples/test-01/`（《Dear Player 2》Master，返工后）待用户评谱，评语按 AGENT 准则 5 记入 notes/knowledge。
