...
- [ ] **待用户拍板**：① CC BY 4.0 是否纳入依赖白名单（决定 SongFormer 去留）；② 知识 002「副歌全踩人声」仍存疑需听审；③ 手序对齐余项：064 口径下 `403-最終鬼畜妹・一部声` MAS m060–063 / `314-FLOWER` Re:MAS m071–072 算不算出张/反手（用户 2026-09-20 指示暂不问，agent 按两口径并列记录）；④ 知识 068–081 为 agent 观察条目，用户随时可覆盖或废弃。
