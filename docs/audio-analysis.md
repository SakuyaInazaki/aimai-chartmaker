...
> **状态**：v1.2（2026-09-11）。在 v1.1 基础上合并**第一份配对标定证据**：
> `docs/research/audio-chart-calibration.md`（8 首官方 mp3 × 官方谱，其 §8 的 R1–R11 逐条回执）
> + 主会话对该报告的验收裁定。本轮改动集中在 §2.4（offset 反拍 bug）、§4.5–§4.8（权重/高潮/
> 密度锚/标定协议 (E) 项）、§5.2–§5.3（切轨模型换成**骨架 + 点缀**）、§9、§11。
> **口径警示**：标定只有 **n=8**（定数 13.5–14.5、BPM 128–230、全 ST MASTER），
> 且切轨侧的 ground truth 是**代理判据**（官方 note 是否落在某 stem 的 librosa onset ±30 ms 内）。
...
4) 融合（新式）：
   ...
   初值 w = [0.25, 0.30, 0.20, 0.15, 0.10]      🧪 已标定过，结论=维持初值（见下）
...
**🧪 v1.2 新增：权重标定结论 —— 不换（R3，标定报告 §3）**

用 8 首官方音频 × 官方谱做非负最小二乘 + **留一曲交叉验证（8 折）**，目标是逐小节官方密度：

| 权重方案 | CV 均值 ρ | CV 中位 ρ |
|---|---:|---:|
| **initial（现行默认，不变）** | 0.488 | 0.518 |
| calibrated（全量 NNLS 拟合） | 0.515 | 0.537 |
| `voiced_to_flux`（H1+H2） | 0.515 | **0.559** |

- **决定：维持初值 `[0.25, 0.30, 0.20, 0.15, 0.10]`。** 依据：CV 均值只 +0.027、
  **5/8 折胜出、符号检验 p ≈ 0.36**，不显著；样本仅 8 首，全量拟合把 68% 权重压在 `flux` 上
  更像过拟合；输掉的那一折（Back 2 Back）反而输最多（−0.186）。
- **两条待验假设**（拿到 **≥20 首**配对数据后优先检验，**不得凭 n=8 改动**）：
  - **H1**：`voiced` 权重 0.15 → **0**（它与逐小节官方密度的 Spearman 只有 **0.016**）。
    口径注意：这只否定"人声活动率能预测**密度**"，不否定人声是情绪载体，也没测它对**配置复杂度**的预测力。
  - **H2**：`flux` 权重 0.10 → **0.25**（单项 ρ **0.524**，是五项最高，**比整条融合式 0.454 还高**）。
- ⚠️ 另一条实测：**平滑没有帮助**（raw ρ 0.468 ≥ smoothed 0.454）——中值+高斯平滑抹掉的正是
  官方谱也会有的单小节爆发（知识 031：峰值小节 ≈ 2.06 × 全曲均值）。目前仍保留平滑，
  但 `bar_raw` 必须同时输出。
...
8 首配对实测（4 小节滑动均值 argmax，与 `chart_analysis` 同口径）：

| 指标 | 实测 |
|---|---|
| 音频强度峰 vs 官方谱密度峰的**位置中位误差** | **33 小节**（曲长 67–124 小节 → 约等于随机猜） |
| 换 8 小节窗 | 34 小节，无改善 |
| 音频强度峰落在谱面密度**前 20% 小节**里 | 4/8 |
| **五票 climax** 落在前 20% 里 | **5/8**（投票机制确实优于裸能量峰，但仍不可靠） |

- 谱面峰位置本身符合知识 031（相对位置中位 0.93、明显偏后）；**是音频强度峰太散**（0.19–0.97）。
...
**🧪 v1.2：地板的 n=8 分歧（R5，标定报告 §4.1）——默认不改，改成可调参数**

| 尺度 | 388 谱给的默认 | n=8 池化最优 | 默认值相对最优的 SSE 劣化 |
|---|---:|---:|---:|
| 小节尺度 | 0.25 | 0.020 | +2.4% → **保留** |
| 段落尺度 | **0.60** | **0.125** | +10.3% → **有分歧** |

- **决定：默认仍 0.60**（388 谱 vs 8 首，以大样本为准），但 CLI 开 `--section-floor` 让人自试
  0.125，song sheet 的 Header 一并注明这个分歧。
- **两个口径不能混**：知识 031 的 0.62 是**谱面自身**段间落差的统计（"官方谱不会把一段写得很空"），
  而这里的 floor 要回答的是"**音频弱到什么程度时谱面才可以变稀**"。标定报告把二者的口径错配
  显式指了出来——这是 0.60 与 0.125 差这么远的原因，不是简单的"谁对谁错"。
- 实测画像（I 五分位，以曲内均值为 1，8 曲平均）：**0.598 / 0.945 / 1.081 / 1.136 / 1.239**，
  最弱档比 `floor=0.25` 模型预测的 0.721 **更低** → 真实映射比模型稍陡。
...
```
suggested(intro/outro) ≤ cap_ratio × mean(全曲 suggested)     # 默认 cap_ratio = 0.60
```

段落级与逐小节级同时封顶，**只封不抬**。⚠️ `0.60` 是从一首曲子推广出来的数，方向有依据、
数值没有；`--intro-outro-cap` 可调，设 0 关闭。
...
- **换锚的原因**：旧锚在 **BPM ≥ 200 的三首（激唱 / 幻想 / 麒麟）上系统性高估 3.1–3.7 note/小节**
  ——note/小节在高 BPM 下等价于更高的 NPS。标定报告口径：**MAPE 21.0% → 16.5%**
  （幻想从 −35% 改善到 −8%）；本机 v0.3 复算（定数取最近档、全曲均值口径）**29.3% → 22.5%**。
- ⚠️ **不是每首都更准**：MYTHOS（10.10 → 11.70，实际 9.67）与 Signature 反而变差。
- ⚠️ **精度只到档位**：逐小节 MAE 2.07–4.69（均值 3.4 note/小节，相对误差 30–45%）
...
- **(D) 音频侧人工听审**：选 5–10 首目标域曲逐段听审给强度档（1–5），与 `I_bar` 的段落均值算 Spearman 相关。
  ❓ 仍未做。**目标值按 (E) 的实测下调**：原写"目标 ρ ≥ 0.8"，而配对数据给出的真实水平只有
  逐小节 0.454 / 逐段 0.523 —— 人工听审的合格线应定在**逐段 ρ ≈ 0.5–0.6** 这个量级，
  ρ ≥ 0.8 是不现实的期待。人工听审的价值转为**查错**（找音频侧明显判反的段落），不是刷相关系数。
- **(E) 配对标定 —— 🧪 已做（n=8，2026-09-11）**：用户提供 8 首官方 ST 曲包（官方 mp3 + 官方
  `&inote_5`），工具 `tools/calibration/`，报告 `docs/research/audio-chart-calibration.md`。主要数字：

  | 项 | 实测（8 首汇总） |
  |---|---|
  | 逐小节 `I_bar` vs 官方 `d_bar` Spearman | **0.454**（smoothed）/ 0.468（raw） |
  | **逐段** Spearman | **0.523** |
  | 单特征最强项 | **`flux` 0.524**（高于整条融合式）；`voiced` **0.016**（无预测力） |
  | 候选池（量化落格后的 onset）对官方 note 的 **recall** | **0.821** |
  | 候选池 **precision**（候选里被官方采用的比例） | **0.546** —— 官方谱只用一半 |
  | 音频段落边界命中官方密度变点 | ±1 小节 **53%**、±2 小节 72%（本次表现最好的一项，但口径对音频有利：平均 9.6 个边界去命中 4 个变点，精确率口径只有 ~22%） |
  | 音频峰 vs 谱面密度峰位置 | 中位误差 **33 小节**（见 §4.6） |
  | 权重 LOSO 交叉验证 | 标定 0.515 vs 初值 0.488，5/8 折，p≈0.36 → **维持初值**（见 §4.5） |

  **结论一句话**：音频强度够用来定"这一段大概多密"，**不够用来定"高潮在哪一小节"**。
  ⚠️ n=8、且切轨侧是代理判据 —— 后续拿到 ≥20 首应复验 H1/H2 与 §5.2 的条件触发阈值。
...
**v1.1 的单值 `primary_stem` 模型已被配对标定证伪**（标定报告 §5，n=8、代理判据）：

| 实测 | 数字 |
|---|---|
| 官方 note 落在 `drums` onset 上（recall / lift） | **0.607 / 2.64**（各 stem 最高） |
| **不**落在 drums 上 | **36.2%** |
| 只落 drums / 只落 vocals | **28.1% / 3.2%** |
| 什么 stem 都不落（自由发挥 / 装饰 / onset 漏检，三者无法区分） | **18.6%** |
| 逐段"实测最高命中轨"分布（85 段） | drums 70 / other 10 / vocals 3 / bass 2 |
| v1.1 规则表与实测最高命中轨的一致率 | **64.7%**，而"**永远答 drums**"是 **82.4%** |

→ 官方谱的形态是 **"鼓骨架 + 大量非鼓的填充与装饰"**，**没有任何一段是"只踩一条轨"**。
所以模型改成：**`skeleton`（骨架轨）+ `accent[]`（点缀轨）+ `share`（估计占比，含 `free` 残差）+ `evidence`（依据）**。
...
| 4 | `chorus`/`final_chorus` | **人声主导** = `voiced_ratio ≥ 0.45` **且** `n_onset_vocals / n_onset_drums ≥ 0.65` | **`drums`（不变）** | 人声主导 → **`vocals` 为主**；否则 `vocals` 只作重音点缀 | **条件触发 + 存疑**（C1，见下）/ 5.4-4 |
| 5 | `interlude` | `voiced_ratio ∈ [0.12, 0.55]` ∧ `n_onset_vocals ≥ 1`/小节 | **`drums`** | **有人声采样 → `vocals` 优先** | **加强**（C4：interlude 的 vocals precision **0.715**、lift **3.18** 均为各段落类型最高——人声在间奏里一旦出现几乎必被采用）；强度靠位移不靠密度（知识 003-2）/ 5.4-5 |
| 6 | 休息段 / `quiet_chorus` | 位于两个高强度段之间 ∧ `I_bar < P40` | **`drums`** | 非鼓里倾向最高者 | **标存疑**（C5：13 段样本、一致率 0.62，数据不支持也不反对）；密度降一档但不清零（知识 031）/ 5.4-6 |
...
- 实测：14 个 `chorus` 段里**只有 2 段** vocals 命中率最高；副歌整体 drums 0.583 vs vocals 0.200；
  `final_chorus` 更极端（vocals recall 0.065、lift 0.97 = 随机水平）。唯一正例是
  **幻想のサテライト**（东方人声 arrange）。
- **但结论只降级、不推翻**：ground truth 是"官方 note 是否落在 Demucs vocals 轨的 librosa onset
  ±30 ms 内"这个**代理**，而人声 onset 检测是全管线最弱环节之一（vocals 轨混 lead synth、
  鼓与人声重合无法区分、人声 onset 数本身构成天花板）。
- → 规则改为**条件性（人声主导曲目）+ 存疑，待人工听审复核**；**不得当硬约束**。
- ⚠️ 阈值 `0.65` 是在这 8 首上分出来的（幻想的副歌 0.68–0.87 vs 其余 7 首 ≤ 0.64），
  **n=8 调参，属存疑**。
...
- **估计占比**：骨架 + 点缀按匹配倾向归一，整体 × (1 − 0.19)，余下 `free = 0.19` 是实测
  "什么都不落"的那 18.6%（留给谱师自由发挥）。⚠️ 逐段占比**从未被验证**，只是量级提示。
...
5. **（v1.2 新增）人声侧的 ground truth 只有代理，需人工听审**：§5.2 C1「副歌全踩人声」被降级为
   条件性，靠的是"官方 note 是否落在 vocals 轨 onset ±30 ms 内"这个代理判据 ——
   而 A.3 已把 vocals 轨的 synth 泄漏列为最不可靠环节之一，人声 onset 数本身还构成命中率天花板
   （8 首里 MYTHOS 的天花板只有 0.104）。**这条结论必须由人工听审复核**，或等 basic-pitch /
   RMVPE 装上后拿真正的人声 note 事件复验。

**B. 未标定 / 未实现**

6. **权重仍是初值，但已标定过一次** 🧪：强度融合 `w=[0.25,0.30,0.20,0.15,0.10]` 在 8 首配对数据上
   做过 LOSO 交叉验证，结论是**维持初值**（+0.027、5/8 折、p≈0.36，§4.5）；投票权重
   `[0.45,0.25,0.20,0.05,0.05]` **完全未标定**。两条待验假设 H1（`voiced`→0）/ H2（`flux`→0.25）
   **须等 ≥20 首配对数据**。(D) 人工听审 ❓ 仍未做，且目标值已从 ρ≥0.8 下调到逐段 0.5–0.6。
7. **（v1.2 新增）几个新引入的阈值只有 n=1–8 的依据** ❓：`intro/outro` 结构封顶比例 0.60
   （来自 MYTHOS 一首）、「人声主导」判据 0.45/0.65（在 8 首上分出来的）、段落 floor 的
   0.60↔0.125 分歧（388 谱 vs 8 首）——全部**参数化并标注**，不得当定论。
...
11. **域外泛化**：所有模型基准都是西方流行乐；🧪 已跑 8 首官方日系曲 + 3 首测试曲，样本仍不够 → 按 §10 的 A/B 档做 20–50 首人工核对。
12. **器乐曲（无副歌）**："副歌=重复最多段"失效，高潮 fallback 为能量峰值（能量票权重升到 0.5）。
13. **官方 maimai 音频**：🧪 **已有 8 首**（用户 2026-09-11 提供，`official-mp3/`，不入库），
   配对标定 §4.8(E) **已完成**；**n=8 太小**，结论一律只作倾向，拿到 ≥20 首后须复验。
14. **密度先验的样本边界**：知识 031 只覆盖 ST 谱定数 13.0–14.5，12 级及以下与 DX 谱外推需谨慎；
   8 首配对曲在密度分位上比全库更"尖"（峰归一 p10/p50/p90 = 0.155/0.454/0.678 vs 全库
   0.264/0.514/0.730），**不足以修订知识 031，只作样本内偏差记录**。
...
### 11.1 `tools/audio_analysis`（原型 **v0.3**，2026-09-11；v0.1 见 note 035、v0.2 见 note 038、v0.3 见 note 041）
...
测试：`tests/test_audio_analysis.py` **81 用例**（全仓 `pytest tests/` **164 通过**），**全部用 numpy 合成音频**（仓库不放真实音频，AGENT 准则 5）。
...
| `intensity.py` | §4.5 五项融合式 + §4.6 五票（**v0.3 正名 `audio_climax_bar`**）+ §4.7 密度地板映射（**v0.3：NPS 主锚 + `--section-floor`/`--bar-floor` + intro/outro 结构封顶**） | ✅（权重为初值，标定结论=不换） |
| `stemplan.py` | §5.2 **v0.3 骨架轨 + 点缀轨 + 估计占比 + 依据**（副歌条件触发并标存疑、间奏加强、休息段标存疑）+ 降级 / 单轨警告 | ✅（阈值 n=8，存疑） |
...
- §4.8(B) 分位映射 `F_chart⁻¹∘F_audio`（当前只有 §4.7 的地板线性映射 + NPS 锚）；(D) 人工听审校验 ❓；
  **(E) 配对标定已完成（n=8）**，但须在 ≥20 首上复验；
- basic-pitch 人声 note 事件（§3.5，`resolution-too-deep` 装不上）——它同时是 C1（副歌踩人声）
  复验的前置；SongFormer 二号意见（§4.2）；
- 器乐向阈值（voiced 能量占比 0.18）、chorus 护栏、器乐曲强度分档阈值只在 3 首上试过，无实证；
- v0.3 新引入的阈值（结构封顶 0.60、人声主导 0.45/0.65）只有 n=1–8 的依据。
