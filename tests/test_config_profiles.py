...
def test_矩阵两份榜():
    rows = ([{"pool_tier": "密", "I_tier": "高", "configs": ["稀有"], "mode": "半采",
              "coverage": 0.5}] * 12
            + [{"pool_tier": "密", "I_tier": "高", "configs": ["常见"], "mode": "全采",
                "coverage": 0.9}] * 20
            + [{"pool_tier": "稀", "I_tier": "低", "configs": ["常见"], "mode": "全采",
                "coverage": 0.9}] * 200)
    ...
    assert cell["top_modes"][0]["mode"] == "全采"
    assert 0.5 < cell["coverage"] < 0.9


def test_矩阵按min_count挡掉小样本():
    rows = ([{"pool_tier": "密", "I_tier": "高", "configs": ["稀有"], "mode": "半采"}] * 3
            + [{"pool_tier": "密", "I_tier": "高", "configs": ["常见"], "mode": "全采"}] * 30)
    m = cpf.recommend_matrix(rows, min_count=10)
    assert [c["config"] for c in m["密|高"]["top_configs"]] == ["常见"]


def test_外在放置画像():
    rows = [{"configs": ["甲"], "I_bar": 0.9, "I_tier3": "高", "mode": "半采",
             "coverage": 0.5, "pos": 0.8},
            {"configs": ["甲"], "I_bar": 0.8, "I_tier3": "高", "mode": "全采",
             "coverage": 0.9, "pos": 0.6},
            {"configs": ["乙"], "I_bar": 0.1, "I_tier3": "低", "mode": "静默",
    ...
    assert abs(pl["甲"]["mode_半采"] - 0.5) < 1e-9
